...
const syncSpec: ToolSpecDefinition = {
	...
	description:
		'Synchronize one linked local file with its Rewst template. Identify the file by the path from buddy_search_template_links or its file URI; orgId must be the org the file is linked to (buddy_template_sync_status returns it). direction defaults to "auto": it uploads when only the local file changed, downloads when the local file is empty, refreshes metadata when the bodies already match, and on a conflict (both changed since the last sync) it changes nothing and asks you to choose. Pass direction:"upload" to overwrite the Rewst template with the local file, or direction:"download" to overwrite the local file with the Rewst template. Requires a connected VS Code editor. Every direction (including download) requires write tools enabled and the org in the effective allowed scope. External MCP calls use client tool permissions; built-in editor uploads retain host approval. Only uploading changes Rewst; downloading and metadata refreshes rewrite local state only.',
