...
export default class RewstFS implements vscode.FileSystemProvider {
	...
	async stat(uri: vscode.Uri): Promise<vscode.FileStat> {
		...
		const entry = await this.tree.lookupEntry(uri);
		log.info(
			`[SEARCH DEBUG] stat() found entry: ${entry.label} (type: ${entry.type}, initialized: ${entry.initialized})`,
		);
		return entry;
	}
	...
	async readFile(uri: vscode.Uri): Promise<Uint8Array> {
		...
		const entry = await this.tree.lookupEntry(uri);
		log.info(
			`[SEARCH DEBUG] readFile() found entry: ${entry.label} (type: ${entry.type}, initialized: ${entry.initialized})`,
		);
	...
	async move(source: vscode.Uri, destination: vscode.Uri): Promise<void> {
		...
		if (source.toString() === destination.toString()) {
			log.error(`Can't move item into itself ${source.toString()}`, false, true);
			return;
		}
