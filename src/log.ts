...
class Logger {
	...
	error(message: any, showToUser = false, raise = false) {
	...
	private log(level: string, message: any, showToUser: boolean) {
		...
		try {
			...
			this.output.appendLine(`[LOGGER ERROR] Failed to write to log file: ${err}`);
		}

		if (showToUser) {
			if (level === 'ERROR') {
				vscode.window.showErrorMessage(typeof message === 'string' ? message : 'Error occurred (see log)');
			} else {
				vscode.window.showInformationMessage(typeof message === 'string' ? message : 'See log for details.');
			}
		}
	}
